/* Toko X yang sedang melakukan SALE ingin menghitung jumlah profit untuk setiap jenis barang yang terjual 
pada hari itu.
...
function countProfit(shoppers) {
  ...
  for (var a = 0; a < listBarang.length; a++) {
    ...
    for (var b = 0; b < shoppers.length; b++) {
      if (profit.product == shoppers[b].product) {
        if (profit.leftOver >= shoppers[b].amount) {
          profit.product = listBarang[a][0];
          profit.shoppers.push(shoppers[b].name);
          profit.leftOver = profit.leftOver - shoppers[b].amount;
          profit.totalProfit = profit.totalProfit + shoppers[b].amount * listBarang[a][1];
        }
      }
    }
    hasil.push(profit);
...
{name: 'Vanessa', product: 'Sepatu Stacattu', amount: 3}, {name: 'Rani', product: 'Sweater Uniklooh', amount: 2}]));
